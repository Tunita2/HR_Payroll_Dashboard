...
const SalaryTable = () => {
  ...
  const fetchSalary = async () => {
    try {
      setLoading(true);
      const res = await axios.get('payroll/salaries');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`payroll/salaries/${selectedEmployeeId}`, selectedEmployeeData);
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      // Gọi API thêm lương
      const response = await axios.post('payroll/salaries/adding', salaryData);
